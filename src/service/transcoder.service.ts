...
import { ITranscoderRepository } from "../interface/iTransRepository";
...
export class TranscoderService implements ITranscoderService {
  ...
  async transcodeMedia(file: File, id: string) {
    try {
      ...
      const videoUrl = `https://instructor-data-bucket.s3.ap-south-1.amazonaws.com/media/hls/${fileName}/${fileName}_master.m3u8`;
      const subtitleUrl = `https://instructor-data-bucket.s3.ap-south-1.amazonaws.com/media/vtt/${fileName}.vtt`;
